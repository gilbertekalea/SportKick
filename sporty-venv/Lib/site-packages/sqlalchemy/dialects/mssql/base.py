...
class MSExecutionContext(default.DefaultExecutionContext):
    ...
    def get_result_cursor_strategy(self, result):
        if self._result_strategy:
            ...
            return super(MSExecutionContext, self).get_result_cursor_strategy(result)
    ...
    def _row_limit_clause(self, select, **kw):
        ...
        if self.dialect._supports_offset_fetch and not self._use_top(select):
            ...
            if limit is not None:
                text += "\n FETCH FIRST %s ROWS ONLY" % self.process(limit, **kw)
            return text
        else:
            return ""
    ...
    def get_indexes(self, connection, tablename, dbname, owner, schema, **kw):
        ...
        rp = connection.execution_options(future_result=True).execute(
            sql.text(
                ...
                "and ind.is_primary_key=0 and ind.type != 0" % filter_definition
            )
            .bindparams(
        ...
        for row in rp.mappings():
            if row["index_id"] in indexes:
                if row["is_included_column"]:
                    indexes[row["index_id"]]["include_columns"].append(row["name"])
                else:
                    indexes[row["index_id"]]["column_names"].append(row["name"])
        for index_info in indexes.values():
            # NOTE: "root level" include_columns is legacy, now part of
            #       dialect_options (issue #7382)
            index_info.setdefault("dialect_options", {})["mssql_include"] = index_info[
                "include_columns"
            ]
