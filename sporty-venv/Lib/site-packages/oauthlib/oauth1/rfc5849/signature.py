...
def signature_base_string(
    http_method: str, base_str_uri: str, normalized_encoded_request_parameters: str
) -> str:
    """
...
def _verify_hmac(
    hash_algorithm_name: str, request, client_secret=None, resource_owner_secret=None
):
    ...
    signature = _sign_hmac(
        hash_algorithm_name, sig_base_str, client_secret, resource_owner_secret
    )
...
def sign_hmac_sha1_with_client(sig_base_str, client):
    return _sign_hmac(
        "SHA-1", sig_base_str, client.client_secret, client.resource_owner_secret
    )
...
def sign_hmac_sha1(base_string, client_secret, resource_owner_secret):
    ...
    """
    warnings.warn(
        "use sign_hmac_sha1_with_client instead of sign_hmac_sha1", DeprecationWarning
    )
    ...
    base_string = (
        base_string.decode("ascii") if isinstance(base_string, bytes) else base_string
    )

    return _sign_hmac("SHA-1", base_string, client_secret, resource_owner_secret)
...
def sign_hmac_sha256_with_client(sig_base_str, client):
    return _sign_hmac(
        "SHA-256", sig_base_str, client.client_secret, client.resource_owner_secret
    )
...
def sign_hmac_sha256(base_string, client_secret, resource_owner_secret):
    ...
    return _sign_hmac("SHA-256", base_string, client_secret, resource_owner_secret)
...
def sign_hmac_sha512_with_client(sig_base_str: str, client):
    return _sign_hmac(
        "SHA-512", sig_base_str, client.client_secret, client.resource_owner_secret
    )


def verify_hmac_sha512(
    request, client_secret: str = None, resource_owner_secret: str = None
):
...
def _sign_rsa(hash_algorithm_name: str, sig_base_str: str, rsa_private_key: str):
    ...
    if not rsa_private_key:
        raise ValueError(
            "rsa_private_key required for RSA with "
            + alg.hash_alg.name
            + " signature method"
        )
...
def sign_rsa_sha1_with_client(sig_base_str, client):
    ...
    sig_base_str = (
        sig_base_str.decode("ascii")
        if isinstance(sig_base_str, bytes)
        else sig_base_str
    )
...
def sign_rsa_sha1(base_string, rsa_private_key):
    ...
    """
    warnings.warn(
        'use _sign_rsa("SHA-1", ...) instead of sign_rsa_sha1', DeprecationWarning
    )
