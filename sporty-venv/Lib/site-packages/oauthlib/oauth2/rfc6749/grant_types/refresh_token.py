...
class RefreshTokenGrant(GrantTypeBase):
    ...
    def __init__(self, request_validator=None, issue_new_refresh_tokens=True, **kwargs):
        super().__init__(
            request_validator,
    ...
    def validate_token_request(self, request):
        ...
        if request.refresh_token is None:
            raise errors.InvalidRequestError(
                description="Missing refresh token parameter.", request=request
            )
        ...
        log.debug(
            "Validating refresh token %s for client %r.",
        ...
        if not self.request_validator.validate_refresh_token(
            ...
            log.debug(
                "Invalid refresh token, %s, for client %r.",
                request.refresh_token,
                request.client,
            )
        ...
        if request.scope:
            ...
            if not all(
                ...
                log.debug(
                    "Refresh token %s lack requested scopes, %r.",
                    request.refresh_token,
                    request.scopes,
                )
