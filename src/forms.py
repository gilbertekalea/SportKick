...
from src.model.schemas import Team, User, Attributes
...
class RegistrationForm(FlaskForm):
    ...
    first_name = StringField(
        label="First Name:", validators=[Length(20), DataRequired()]
    )
    last_name = StringField(label="Last Name:", validators=[Length(20), DataRequired()])
    team_name = SelectField(
        label="Team Name:", choices=teams, validators=[Length(min=4), DataRequired()]
    )
    ...
    experience = SelectField(
        label="Select Experience:", choices=EXPERIENCES, validators=[DataRequired()]
    )
...
class UserCreateAccountForm(FlaskForm):
    ...
    first_name = StringField(
        label="First Name:", validators=[Length(min=2, max=15), DataRequired()]
    )

    last_name = StringField(
        label="Last Name:", validators=[Length(min=2, max=15), DataRequired()]
    )
...
class CreatePostForm(FlaskForm):
    post_title = StringField(
        label="Title:", validators=[Length(min=15, max=50), DataRequired(0)]
    )
    post_content = TextAreaField(
        label="Description:", validators=[Length(max=1024), DataRequired()]
    )
