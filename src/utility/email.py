...
class ManageEmailTemplate(UserRelevantData):
    ...
    env = Environment(
        loader=PackageLoader("src", "templates\emails_templates"),
    ...
    def email_channel_composer(self) -> Message:
        if self.name == "registration":
            msg = Message(
                ...
                html=self.get_sport_registration_email_template().render(
                    username=self.username, meta_data=self.meta_data
                ),
                # , profile=f'https:http://127.0.0.1:5000/user'),
                charset="utf-8",
            )
            return msg
            ...
            msg = Message(
                ...
                html=self.get_account_creation_email_template().render(
            ...
            flash(
                "Unfortunately, we do not understand that command.", category="danger"
            )
